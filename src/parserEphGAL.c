...
int parseEph_Gal(const char* file)
{
  ...
  while ( !feof(fd) )
  {
   ...
   SISerr_Gal[i-1].systype = systype;
    ...
    fscanf(fd, "%lf", &(SISerr_Gal[i-1].M0_SRE));//22  123.123  - СКП R, м                 МAX ошибки R, м           95% модуля ошибки R, м
    ...
    fscanf(fd, "%lf", &(SISerr_Gal[i-1].SISRE));//26  123.123  - СКП SISRE, м             МAX ошибки SISRE, м       95% модуля ошибки SISRE, м
    ...
    fscanf(fd, "%lf", &(SISerr_Gal[i-1].SISVE));//36  123.123  - СКП SISVE, мм/с          МAX ошибки SISVE, мм/с    95% ошибки SISVE, мм/с
   fscanf(fd, "%c", &systype);
  }
  int imax=i;
  fclose(fd);
