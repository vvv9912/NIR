...
int parseEph_GLNS(const char* file)
{
  ...
  while ( !feof(fd) )
  {
   ...
   SISerr_GLNS[i-1].systype = systype;
    ...
    fscanf(fd, "%lf", &(SISerr_GLNS[i-1].M0_SRE));//21  123.123  - МО ошибки SISRE, м       МIN ошибки SISRE, м       Med модуля ошибки SISRE, м
    ...
    fscanf(fd, "%lf", &(SISerr_GLNS[i-1].SISRE));//26  123.123  - СКП SISRE, м             МAX ошибки SISRE, м       95% модуля ошибки SISRE, м
    ...
    fscanf(fd, "%lf", &(SISerr_GLNS[i-1].SISVE));//36  123.123  - СКП SISVE, мм/с          МAX ошибки SISVE, мм/с    95% ошибки SISVE, мм/с
   fscanf(fd, "%c", &systype);
  }
  int imax=i;
  fclose(fd);
