...
% pars(dataB,4);
 function p = pars(dataGa,nfigure)
  for (i = 1:length(dataGa))
     if (abs(dataGa(i,2))>10)
   dataGa(i,2)=0;
   dataGa(i,3)=0;
   end
    if (abs(dataGa(i,3))>10)
   dataGa(i,2)=0;
   dataGa(i,3)=0;
    end
  end
...
for (i = 1:length(dataGa)-20001)
    ...
    if (i == 1)
     meann(1,k) = mean(dataGa(i:1:i+15000,1));
     meann(2:4,k) = mean(dataGa(i:1:i+15000,2:4));
%      stdd(1,k) = std(dataGa(i:1:i+20000,3));
      k =k+1;
    end
if (mod(i,20001) == 0)
     meann(1,k) = mean(dataGa(i+1:1:i+15000,1));
     meann(2:4,k) = mean(dataGa(i+1:1:i+15000,2:4));
...
plot(meann(1,:),sko);
if (nfigure == 10)
     legend('GALILEO','ГЛОНАСС','GPS','Location','southeast' );
     name = 'all_';
      ymin = 0;
      ymax = 4;
     y = [ymin ymax];
...
xlabel('Широта, градусы')
ylabel('3*\sigma, м');
ylim(y);

set(gca, 'YTick',ymin:shag2:ymax)
...
% %  function p = pars(dataGa,nfigure)
% % zero =0;
% % %зануляет сингул и в обалсти не видимости
% % for (i = 1:length(dataGa))
% % if (dataGa(i,9)  == -10)
% %     dataGa(i,9:11) =0;
% %     zero = zero +1;
...
% % k = 1;
% % for (i = 1:length(dataGa)-20001)
% % %     if (abs(dataGa(i,9))>0.5)
% % %     dataGa(i,9) =0;
% % %     dataGa(i,10) =0;
% % %     end
% % %     if (abs(dataGa(i,10))>0.5)
% % %     dataGa(i,9) =0;
% % %     dataGa(i,10) =0;
% % %     end
% %     
% %     if (i == 1)
% %      meann(1,k) = mean(dataGa(i:1:i+15000,3));
% %      meann(2:3,k) = mean(dataGa(i:1:i+15000,9:10));
% % %      stdd(1,k) = std(dataGa(i:1:i+20000,3));
% %       k =k+1;
% %     end
% % if (mod(i,20001) == 0)
% %      meann(1,k) = mean(dataGa(i+1:1:i+15000,3));
% %      meann(2:3,k) = mean(dataGa(i+1:1:i+15000,9:10));
% % %      stdd(1,k) = std(dataGa(i+1:1:i+20001,3));
% %       k =k+1;
% % end
% % end
...
% hold on;
% grid on;
% legend('GALILEO','ГЛОНАСС','GPS','Location','northwest' );
% m2 = "B";
% jpge = ".jpg";
% name = "all"
